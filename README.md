...
- `airtable-client.html` / `netlify/functions/airtable-client.js`
  - 工作台「案件與溝通 → 客戶資料管理」可依客戶編號、公司或聯絡人搜尋，並新增及修改 Airtable【DGHM】Base 的客戶與聯絡人；進入頁面時不載入名單，第一次搜尋才讀取資料，清單每頁顯示 10 筆，採用工作台選單、客戶搜尋結果與詳細編輯三欄式，右側以公司／聯絡人頁籤切換；首頁也有快速入口。
  - 需要先登入工作台。Airtable PAT 只放在 Netlify 環境變數 `FOR_AIRTABLE_DGHM_BASE`，需有該 Base 的 `data.records:read` 與 `data.records:write` 權限；不要把 PAT 放進 HTML 或提交到 Git。
...
- 報價單 → 請款單交接
  ...
  - 一次付清（只有一期）時，請款單項目改為把報價的計價表／勾選方案表
    ...
    不猜，仍沿用整期一個項目的做法；這個整期一個項目的說明欄改列出
    報價單各區塊逐列的項目名稱（`invoiceItemSummary()`），不再只寫
    區塊標題——報價單改用「說明表」（不拆單項金額）呈現是常態做法，
    這種情況下標題摘要（如「一、專案範圍與規格」）太籠統，看不出實際
    請款是要請什麼服務。
...
- 報價單 → 合約文件交接
  - `Quote-Generator.html` 的「產生合約 →」把客戶（甲方）資料、專案名稱、
    服務範圍摘要、工期與**完整**付款期別，透過 localStorage 的
    `dghm-quote-to-contract` 交給 `Contract-Generator.html`。跟交給
    請款單不同——請款單一次只挑一期，合約要放完整付款排程，所以按下去
    直接交接，不會跳出期別選擇對話框。
  - 每期金額都能換算成數字時才會算出「總金額」與大寫金額（沿用
    `Contract-Generator.html` 自己的 `ntdToChinese()`）；只要有一期是
    百分比之類無法換算的寫法，總金額整欄留空，付款期別仍會照樣帶入
    （期別／款項名稱／時點／能換算的金額），不會因為一期算不出來就整批
    不帶。
  - 期別文字若含「·」會拆成「期別」與「款項名稱」兩欄（例如
    「第一期 · 簽約款」→ 期別「第一期」、名稱「簽約款」）；沒有「·」
    就整段當款項名稱，期別自動編號。
  - 交接資料同樣單次取用、超過 1 小時視為過期。
  - **合約條文（articles）現在會自動代入固定／半固定模板**，比對過三份
    實際簽過的合約（DH-05／06／11-PJ1-CT-1）逐條整理：14 條裡有 8 條
    三份逐字相同（驗收方式、保固、著作權歸屬、素材保證、資料保密、
    不可抗力、終止與違約責任、爭議解決），直接照抄；其餘 6 條（委任
    目的、服務範圍、製作期間、費用與付款方式、第三方服務免責、附則）
    句型固定、代入報價單資料（專案名稱、報價單編號、工期、付款期別、
    總金額）。
    - 第四條「費用與付款方式」的中文大寫金額用 `{{TOTAL_WORDS}}` /
      `{{TOTAL_AMOUNT}}` 佔位，交給 `Contract-Generator.html` 用它自己
      既有的 `ntdToChinese()` 換算後取代，避免兩邊各寫一份換算邏輯。
    - 「工期起算方式」（合約生效後起算／以 Kickoff Meeting 為準）、
      素材與功能的具體列舉、第三方服務的實際組合，這些每次都要看案子
      調整，套用最常見的預設寫法，不猜，需要的話請自行改。
    - 服務範圍（`f_scope`）跟著改為固定寫法「依附件一《報價單 XXX》
      所載內容為準」，原本用報價單區塊標題當摘要（跟三份真實合約的
      用字對不起來），改掉了。
    - `Contract-Generator.html` 新增「報價單參照」欄位（`f_qt_ref`），
      顯示於頁首（合約編號、立約日期之後），比照三份真實合約的版型；
      這個欄位原本的資料結構裡完全沒有，交接時一併帶入報價單編號。
    - 付款備註（`payment_note`：訂金到位前不施作、DGHM 匯款帳戶資訊）
      與簽署說明（`f_sig_desc`）也一併固定帶入。
    - **乙方（DGHM 自己）資料固定帶入**（角色、公司名稱、地址、統編、
      聯絡人、電話 / Email），每次交接都直接覆蓋，不依賴瀏覽器暫存裡
      可能存在的舊值——這是固定不變的公司資料，沒有理由每次都要重打。
    - 條文帶過來後仍會秀提示：**這仍是法律文字，套模板只是省去打字，
      不是最終定稿**，開單時務必自行檢視確認。

- 金額欄位補上千分位顯示
  - `Quote-Generator.html` 的計價表／勾選方案表金額欄、付款方式卡片金額，
    與 `Contract-Generator.html` 的付款期別表格金額欄，原本都是原樣輸出
    使用者打的字，沒有千分位（例如打「45000」印出來就是「45000」，不會
    變成「45,000」）。`Invoice-Generator.html` 原本就有格式化，不受影響。
  - `Quote-Generator.html` 新增 `displayAmount()`：純數字（可含逗號、
    `NT$`／`$` 前綴）時補上千分位，其餘原樣顯示（金額欄有時候會填「洽詢」
    「免費」之類的文字，不能一律當數字處理）。
  - `Contract-Generator.html` 付款期別表格金額欄改用既有的
    `formatTotalDisplay()`，跟「含稅總金額」欄位的格式化邏輯一致。
  - 只影響列印預覽的顯示，輸入框裡的原始值不會被改寫，編輯時仍看得到
    自己打的原始文字。
